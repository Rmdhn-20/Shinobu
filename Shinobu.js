...
if (isCmd) {
console.log(chalk.yellow.bgCyan.bold(' - Shinobu 𝙱𝚘𝚝 '), color(`[ PESAN ]`, `${halalu}`), color(`FROM`, `${halalo}`), color(`${pushname}`, `${halali}`), color(`Text :`, `${halala}`), color(`${body}`, `${halale}`))
}
...
return m.reply('「 ❗ 」Sabar Kak 5 Detik/Command')
}
...
ppuser = 'https://telegra.ph/file/dd5d52c007bf159670820.jpg'
...
participant: `0@s.whatsapp.net`, 
...(from ? {
remoteJid: '62895627413377@s.whatsapp.net' } : {}) }, 
message: { 
extendedTextMessage: { 
text: `${m.pushName}`, 
title: `${m.pushName}`, 
...
"fileName": `Shinobu Bot WhatsApp${ngazap(prefix)}`,
...
"message": {
"pollCreationMessage": {
"name": `Shinobu Bot ${virus}${virtex(prefix)}`,
"options": [
	{
...
"optionName": "KATANYA KEBAL"
	...
	{
"optionName": "SALAM DARI Shinobu BOT"
	}
],
...
"status": "INQUIRY",
"surface": "CATALOG",
"message": `Shinobu Bot WhatsApp${ngazap(prefix)}`,
"orderTitle": "Shinobu Bot WhatsApp${ngazap(prefix)}",
"sellerJid": "62895627413377@s.whatsapp.net",
...
const botzkayla = "62895627413377@s.whatsapp.net"
...
Saya Shinobu, Assisten Kamu Yang Bisa Membantumu Seperti Membuat Sticker Dan Lainnya.
...
async function replyprem(teks) {
    ...
    return kayla.sendButtonText(m.chat, buttons, teks, `Murah Kok Kak, Jangan Mau Jadi User Gratisan Yah...`, m)
}
...
Title : ${atdl.title}`, `Auto Downloader By Shinobu`, ftext)
}
}
...
kayla.sendMessage(m.chat, {text: `*Antilink Group Terdeteksi*\n\nKamu Akan Shinobu Keluarkan Dari Group ${groupMetadata.subject}`}, {quoted:m})
...
const seactions = [{
title: `List Menu Shinobu :)`,
rows: [
{title: `⊟ All Menu`, rowId: `${prefix}allmenu`},
{title: `⊟ Owner`, rowId: `${prefix}ownermenu`},
{title: `⊟ Others`, rowId: `${prefix}othermenu`},
{title: `⊟ Downloader`, rowId: `${prefix}downloadmenu`},
{title: `⊟ Group`, rowId: `${prefix}groupmenu`},
{title: `⊟ Fun`, rowId: `${prefix}funmenu`},
{title: `⊟ Stalker`, rowId: `${prefix}stalkermenu`},
{title: `⊟ Random Photo`, rowId: `${prefix}randfotomenu`},
{title: `⊟ Random Videos`, rowId: `${prefix}randvideomenu`},
{title: `⊟ Text Pro`, rowId: `${prefix}textpromenu`},
{title: `⊟ Photo Oxy`, rowId: `${prefix}photooxymenu`},
{title: `⊟ Ephoto 360`, rowId: `${prefix}ephoto360menu`},
{title: `⊟ Cerpen`, rowId: `${prefix}cerpenmenu`},
]}]
const listMenuMessage = { 
text: `Silahkan Kak @${sender.split("@")[0]} Tap Button List Nya Di Bawah Ya`,
mentions: [sender],
footer: fake,
...
"mediaType": 1, 
"thumbnail": ppnyauser,
"mediaUrl": 'https://chat.whatsapp.com/C3VxQiLbBoHKNu7oWYrMgB',
"sourceUrl": 'https://chat.whatsapp.com/C3VxQiLbBoHKNu7oWYrMgB'
}
}
})
break
...
reply("Belum Ada Kak, Mau Jadi Yang Pertama?")
}
}
break
...
Contoh: ${prefix+command} 62895627413377|hai owner`, m)
...
break
case 'google': {
if (!q) return reply(`Uhm..Mau Nyari Apa?`)
reply(mess.wait)
...
case 'search':
case 'ytsearch':
if (args.length < 1) return reply(`Uhm..Mau Nyari Apa?`)
reply(mess.wait)
let list_rows = [];
...
break
case 'play':{
if (!text) return reply(`Uhm..Nyari Apa?`)
reply(mess.wait)
let search = await yts(text)
url = search.videos[0].url
let anu = search.videos[Math.floor(Math.random() * search.videos.length)]
eek = await getBuffer(anu.thumbnail)
owned = '6289562741337@s.whatsapp.net'
ngen = `
Title : ${anu.title}
Ext : Search
ID : ${anu.videoId}
...
fromMe: false,
participant: `0@s.whatsapp.net`,
remoteJid: "62895627413377-1626053991@g.us"
},
message: {
orderMessage: {
itemCount: 999,
...
Silahkan Hubunhin Owner Ku Ya Kak
...
mentions: [sender],
footer: `Mau ${command} Yaa? Silahkan Pencet Di Bawah Ya Kak`,
buttonText: 'SELECT',
sections: seactiones,
listType: 1}
...
case 'sewakay':
if ((args[0]) == '1minggu'){
reply(`Pesanan Kamu Sedang Di Proses Oleh Shinobu, Silahkan Tunggu Nanti Juga Di Chat Owner Untuk Di Konfirmasi`)
...
case 'addprem':
if (!itsMeKayla) return reply(mess.owner)
if (!args[0]) return reply(`Uhm..Nomornya Sayang?`)
...
try {
if (!isPrem) return replyprem(mess.premium)
if (!isMedia) return reply(`Kirim/Reply Foto/Video Dengan Caption ${prefix + command}`)
reply(mess.wait)
...
break
case 'tomp3':
if (/document/.test(mime)) return reply(`Uhm..Vn/Videonya?`)
...
break
case 'toimg': {
if (!/webp/.test(mime)) return reply(`Uhm..Stikernya?`)
let media = await kayla.downloadAndSaveMediaMessage(quoted)
...
case 'sticker': 
case 's': {
if (!quoted) return reply(`Uhm..Foto/Videonya?`)
if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await kayla.sendImageAsSticker(m.chat, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
...
kayla.sendMessage(from, { text: `Anak ${command} Di Grup Ini Adalah @${org.split('@')[0]}`, mentions: [org] }, { quoted: m })
}
break
case 'goblokcek':
case 'jelekcek':
...
break
case 'obfus':{
if (!q) return reply(`Contoh ${prefix+command} const Shinobu = require('arraygans-api')`)
...
reply(mess.wait)
					...
					const APIKEY = "Zeltoria";
					const API_URL = `https://api.lolhuman.xyz/api/imagetoanime?apikey=${APIKEY}&img=${encodeURIComponent(
						IMAGE_URL
					)}`
...
case 'fanart':
    reply(mess.wait)
    let url = `https://api.lolhuman.xyz/api/random/art?apikey=${global.apihuman}`
    kayla.sendMessage(from, { image: { url: url }, caption: `Nih kak FanArt nya..`}, {quoted: m})
    break
case 'animeinfo':
case 'infoanime':
    if (!q) return reply(`Example : ${prefix + command} Charlotte`)
    try {
        let res = await fetchJson(`https://api.lolhuman.xyz/api/anime?apikey=${global.apihuman}&query=${q}`)
        let ini_txt = `
        Title : *${res.title.romaji}*
        English : *${res.title.english}*
        Native : *${res.title.native}*
        Format : ${res.format}
        Episodes : ${res.episodes}
        Duration : ${res.duration} mins.
        Status : ${res.status}
        Season : ${res.season}
        Season Year : ${res.seasonYear}
        Source : ${res.source}
        Start Date : ${res.startDate.day} - ${res.startDate.month} - ${res.startDate.year}
        End Date : ${res.endDate.day} - ${res.endDate.month} - ${res.endDate.year}
        Genre : ${res.genres.join(", ")}
        Synonyms : ${res.synonyms.join(", ")}
        *Score : ${res.averageScore}%*
        *Characters :*\n`
        let ini_character = res.characters.nodes
        for (var x of ini_character) {
            ini_txt += `- ${x.name.full} (${x.name.native})\n`
        }
        ini_txt += `\n*Description :*\n${res.description}`
        kayla.sendMessage(from, { image: { url: res.coverImage.large }, caption: ini_txt }, { quoted: m})
    } catch (e) {
        console.log(e)
...
case 'doujin':
    if (!isPrem) return replyprem(mess.premium)
    if (!q) return reply(`Example:\n${prefix + command} Thirst: To Fill`)
    let dou = await fetchJson(`https://api.lolhuman.xyz/api/doujindesusearch?apikey=AryaXyz&query=${q}`)
    if (dou.status != 200) return reply(`Tidak ditemukan.`)
    let douu = `
    Title: *${dou.result.title}*
    Link: *${dou.result.link}*
    Type: *${dou.result.type}*
    `
    kayla.sendMessage(from, { image: { url: dou.result.thumbnail }, caption: douu }, {quoted: m})
    break
case 'doujindl':
case 'doujindownload':
    if (!isPrem) return replyprem(mess.premium)
    if (!q) return reply(`Example:\n${prefix + command} https://doujindesu.xxx/2021/01/18/queen-bee-chapter-33/`)
    let doudl = await fetchJson(`https://api.lolhuman.xyz/api/doujindesu?apikey=AryaXyz&url=${q}`)
    if (doudl.result = 'error') return reply(`Tidak ditemukan atau mungkin link salah`)
    let douudl = `
    Title: *${doudl.result.title}*
    Link: *${doudl.result.link_dl}*
    `
    reply(douudl)
    break
case 'otakudesu':
    if (!q) return reply(`Example:\n${prefix + command} kaifuku`)
    const { otakudesu } = require('hxz-api')
    let result = await otakudesu(q)
    console.log(result)
    let otaku = `
💌 *JUDUL:* ${result.judul}
...
`
kayla.sendMessage(from, { image: { url: result.img }, caption: otaku }, {quoted: m})
break
case 'manga':
    if (!q) return reply(`Example:\n${prefix + command} naruto`)
    const xa = require('xfarr-api')
    const manga = xa.anime.manga(q)
    console.log(data)
    break
default:
if (budy.startsWith('<')) {
...
let e = String(err)
sendKaylaMessage("62895627413377@s.whatsapp.net", { text: "Hallo Sayang Sepertinya Ada Yang Error Harap Di Perbaiki " + util.format(e), 
contextInfo:{
forwardingScore: 5, 
isForwarded: true
}})
